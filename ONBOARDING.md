...
- `apps/client/src/i18n`
  Локализация клиента (`ru/en`).

Практические UX-заметки по текущему клиенту:

- в `Личных сообщениях` поиск пользователей встроен прямо в верх списка, отдельного `+` dropdown больше нет;
- `Текстовые каналы` и `Голосовые каналы` считаются статичными дефолтными категориями пространства;
- tooltip'ы приведены к общей тёмной палитре OpenCord;
- preview файлов в composer — это компактные file cards/chips, а не широкие placeholder-блоки.
