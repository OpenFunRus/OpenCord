...
- По client UX / dark tooltip polish на 2026-04-23:
  ...
  - базовые tooltip'ы унифицированы в тёмный OpenCord-style, включая реакции, emoji picker, шестерёнку сервера и аватарки пространств;
  - для дефолтных категорий `Текстовые каналы` / `Голосовые каналы` отключено контекстное меню по ПКМ по самому заголовку;
  - в `Личных сообщениях` убран dropdown `+`: сверху списка теперь встроенный search/filter input;
  - preview прикрепляемых файлов в composer переведён на компактные карточки с file-иконкой и крестиком удаления в правом верхнем углу.
