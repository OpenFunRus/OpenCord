...
describe('message content limits', () => {
  test('should calculate text metrics with HTML line breaks', () => {
    expect(getMessageTextMetrics('<p>Hello</p><p>World</p>')).toEqual({
      textLength: 11,
      lineCount: 2
    });
  });

  test('should return null for content inside limits', () => {
    expect(
      getMessageContentLimitError('<p>This message is within limits.</p>')
    ).toBeNull();
  });

  test('should detect length overflow', () => {
    const oversized = 'a'.repeat(MESSAGE_MAX_TEXT_LENGTH + 1);
    expect(getMessageContentLimitError(oversized)).toBe('MAX_LENGTH');
  });

  test('should detect line overflow', () => {
    const tooManyLines = Array.from({ length: MESSAGE_MAX_LINES + 1 })
      .map(() => 'line')
      .join('<br>');
    expect(getMessageContentLimitError(tooManyLines)).toBe('MAX_LINES');
  });
});
