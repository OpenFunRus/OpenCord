...
- В `Личных сообщениях` поиск пользователей теперь встроен прямо в список, без отдельного `+` dropdown.
- Дефолтные категории `Текстовые каналы` / `Голосовые каналы` считаются статичной частью структуры пространства.
