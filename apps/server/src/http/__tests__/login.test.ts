...
describe('/login', () => {
  ...
  test('should auto-register new user when allowNewUsers is true', async () => {
    ...
    expect(newUser?.name).toBe('OpenFun');
  });

  test('should require display name for first-time users', async () => {
    const response = await login('displaynameuser', 'password123');
    ...
    expect(data).toHaveProperty('errors');
    expect(data.errors).toHaveProperty('name', 'Display name is required');
  });
  ...
  test('should assign default role to newly registered user', async () => {
    const response = await login('roleuser', 'password123', undefined, 'RoleUser');
  ...
  test('should assign owner role to the first registered user when no owner exists', async () => {
    ...
    const newUser = await tdb
      .select()
      .from(users)
      .where(eq(users.identity, 'firstowneruser'))
      .get();

    expect(newUser).toBeTruthy();

    const assignedRoles = await tdb
      .select()
      .from(userRoles)
      .where(eq(userRoles.userId, newUser!.id));

    expect(assignedRoles.some((role) => role.roleId === OWNER_ROLE_ID)).toBe(true);
  });
