    ...
    if (pinnedCardId && pinnedCard) {
      return (
        <div className={cn('flex flex-col h-full', className)}>
          <div className="min-h-0 flex-1 p-3">{pinnedCard}</div>

          {regularCards.length > 0 && (
            <div className="flex-shrink-0 border-t border-[#2b3544] bg-[#172231]/92 backdrop-blur-sm">
              <div className="flex justify-center gap-2 overflow-x-auto p-3">
                {regularCards.map((card, index) => (
                  <div key={index} className="h-24 w-40 flex-shrink-0">
                    {card}
                  </div>
                ))}
              </div>
