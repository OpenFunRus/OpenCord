...
### Client UX polish: тёмные tooltip'ы, DM filter и компактные file chips

- Локализация клиента упрощена до двух языков:
  - каталог `apps/client/src/i18n/locales/zh` удалён;
  - актуальная клиентская локаль теперь только `ru/en`.
- Tooltip-слой унифицирован под dark OpenCord-style:
  - базовый `packages/ui Tooltip` переведён на тёмную палитру с аккуратной стрелкой;
  - кнопочные `title`-подсказки в `Button` / `IconButton` завернуты в общий tooltip;
  - реакции, emoji picker, server gear и аватарки пространств больше не используют белые browser-tooltip'ы.
- Доведён sidebar/navigation UX:
  - у space-аватарок появился hover-tooltip с именем пространства без конфликта с context menu;
  - для статичных категорий `Текстовые каналы` / `Голосовые каналы` отключён ПКМ по самому заголовку, чтобы их нельзя было воспринимать как редактируемые;
  - в `Личных сообщениях` убран `+` dropdown поиска пользователей, вместо него сверху списка добавлен встроенный filter input.
- Composer attachments пересобраны под компактный preview:
  - вместо растягивающихся wide-placeholder карточек используются небольшие file chips фиксированной ширины;
  - внутри показываются file-type icon, имя, размер и remove-кнопка `X` в правом верхнем углу.
...
- Доведён визуальный и UX-слой `spaces`:
  - исправлены аватарки пространств, подсветка выбранного пространства и контекстное меню по ПКМ;
  - скрыт мешающий scrollbar у ленты пространств и возвращён drag-scroll мышью;
  - доработаны размеры, отступы и hover/selected-состояния у space-аватарок.
- Модалка создания/редактирования пространства приведена к единому modal-style OpenCord:
  - исправлены смешанные радиусы/обрезания;
  - input-фокус переведён на фирменную синюю палитру вместо серого дефолта.
- Нижняя зона текстового чата и блок пользователя выровнены по общей базовой высоте:
  - composer и user-control синхронизированы по `95px`;
  - счётчики лимитов вынесены в верхнюю строку над полем ввода;
  - в нормальном состоянии счётчики нейтральные, при превышении краснеет только проблемное число.
- Индикатор набора текста локализован:
  - убран хардкод `is typing...`;
  - добавлены ключи `typingSingle / typingDouble / typingMany` для `ru/en/zh`.

### Каналы и дефолтные категории

- Убрана ручная команда `Добавить категорию` из серверного меню-шестерёнки.
- На сервере добавлена гарантия базовой структуры пространств:
  - при создании нового пространства автоматически создаются `Текстовые каналы` и `Голосовые каналы`;
  - при запуске БД выполняется backfill этих категорий для уже существующих пространств.
- Модалка создания канала доработана:
  - дефолтное имя нового канала локализовано;
  - добавлен корректный reset состояния при каждом открытии;
  - модалка переведена на `overflow-hidden`, чтобы убрать баг смешанных острых/скруглённых углов.
- Для дефолтных категорий плюс теперь открывает создание канала сразу в нужном типе:
  - `Текстовые каналы` -> только текстовый канал;
  - `Голосовые каналы` -> только голосовой канал.

## 2026-04-20

### Spaces: новая серверная и клиентская модель навигации

- Полностью убрана правая панель участников:
  - список участников перенесён в `Личные сообщения`;
  - логика списка перестроена в порядке `недавние диалоги -> онлайн -> оффлайн`.
- В проект добавлена система `spaces`:
  - новые таблицы `spaces` и `space_roles`;
  - `categories` и `channels` получили привязку к `spaceId`;
  - добавлены CRUD-роуты, sync-маршруты и pubsub-события для пространств.
- На клиенте добавлены:
  - `selectedSpaceId` в состоянии сервера;
  - фильтрация категорий и каналов по выбранному пространству;
  - strip аватарок пространств в левой панели;
  - модалка создания/редактирования/удаления пространства;
  - новый permission `MANAGE_SPACES`.
- Поиск контента синхронизирован с новой моделью прав:
  - поиск идёт по всем каналам, доступным пользователю по ролям и видимым пространствам;
  - скрытый контент из чужих пространств в результаты не попадает.
- Удалён старый server/client flow управления emoji:
  - router-ветка `emojis` и связанные client-side screens/hooks удалены как legacy-часть текущего OpenCord workflow.

## 2026-03-19

### Текстовый чат: bounded window, jump to present и облегчённый image pipeline

- Текстовая лента пересобрана с отдельного `history mode` на единую модель:
  - видимое окно сообщений остаётся ограниченным;
  - старые сообщения уходят в `beforeBuffer`, более новые во время jump-сценариев — в `afterBuffer`;
  - возврат к текущим сообщениям работает через `jump to present`, а не через отдельный выход из истории.
- Переходы из закрепов и других jump-сценариев переведены на ту же модель окна/буферов, чтобы чат не размонтировался целиком при каждом переходе.
- Для списка сообщений убраны нестабильные групповые `key={index}`; ключи групп теперь строятся по диапазону message ids.
- Рендер изображений в чате облегчён:
  - fullscreen-`img` больше не монтируется скрытым для каждой картинки заранее;
  - превью загружаются ленивее, с `loading=\"lazy\"` и `decoding=\"async\"`;
  - это заметно снизило GPU-нагрузку на каналах с большим числом фотографий.
...
- В базовом `packages/ui Dialog` обновлён close-control:
  - убрано серое свечение в hover/focus-состояниях;
  - восстановлено корректное центрирование `X`.
...
### Chat attachments и tabs: визуальная доводка под стиль OpenCord

- Вложения в текстовом чате доведены по отображению:
  - у изображений убрана лишняя подложка под превью, сохранены overlay-кнопки действий поверх самой картинки;
  - действия разделены по правам: `скачать` доступно всем, `удалить` — автору сообщения и пользователям с `MANAGE_MESSAGES`;
  - сетка картинок и файлов сделана адаптивной по ширине сообщения с ограничением до `3` колонок.
- Кнопка удаления в карточках прикреплённых файлов переведена на единый hover/focus-стиль OpenCord без постороннего коричневого фона.
- Верхние tab-кнопки в `Настройках сервера` и `Настройках пользователя` приведены к общей тёмной палитре OpenCord:
  - базовый фон `#101926`;
  - hover в стиле `#1b2940 / #3d516b`;
...
- Кнопка громкости в карточках пользователей доведена до того же визуального стандарта.
- Поведение fullscreen-кнопки скорректировано: на desktop она показывается как часть hover-контролов, на mobile — показывается по tap как hover-аналог.
- Нижняя панель звонка на mobile переведена на раскрытие по стрелке снизу (tap-to-expand controls).

### UI/UX: тёмные toast-уведомления и русские media-ошибки

- В `packages/ui` обновлена стилизация toast-уведомлений под dark theme с цветными и более жирными текстами для `success/error/info/warning`.
- В client voice-flow добавлена локализация типовых media-device ошибок (`NotFoundError`, `NotAllowedError` и др.) для русских popup-уведомлений.
